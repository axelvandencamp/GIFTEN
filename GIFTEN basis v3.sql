...
--/*
--------------------------------------------------
--QUERY 1a: ophalen ERP Giften uit "CRM boekingen"
----------
INSERT INTO tempGIFTEN
	...
	ORDER BY aml.date);
--------------------------------------------------
--QUERY 1b: ophalen ERP Giften van inactieve dubbels
----------
INSERT INTO tempGIFTEN
	(
		SELECT aml.account_id,
		...
		(credit - debit) amount,
		p6.id partner_id,
		p.id inact_partner_id,
		p6.membership_state huidige_lidmaatschap_status,
		p6.membership_nbr lidnummer,
		'[' || p6.id::text || '] ' || p6.name as partner,
		p6.name as naam,
		p6.first_name as voornaam,
		p6.last_name as achternaam,
		COALESCE(COALESCE(a2.name,a.name),'onbekend') afdeling,
		p6.membership_cancel as opzegdatum,
		CASE
			WHEN c.id = 21 AND p6.crab_used = 'true' THEN ccs.name
			ELSE p6.street 
		END straat,
		CASE
			WHEN c.id = 21 AND p6.crab_used = 'true' THEN p6.street_nbr ELSE ''
		END huisnummer, 
		p6.street_bus bus,
		CASE
			WHEN c.id = 21 AND p6.crab_used = 'true' THEN cc.zip
			ELSE p6.zip
		END postcode,
		CASE 
			WHEN c.id = 21 THEN cc.name ELSE p6.city 
		END gemeente,
		p6.postbus_nbr postbus,
		CASE WHEN COALESCE(c.id,0) = 21 AND p6.zip = '2070' THEN 'Oost-Vlaanderen'
			WHEN COALESCE(c.id,0) = 21 THEN cs.name
			WHEN COALESCE(c.id,0) = 166 THEN 'Nederland'
			ELSE 'Buitenland niet NL' 
		END provincie_state,
		...
		p6.birthday,
		a5.name partner_naam,
		COALESCE(p6.address_state_id,0) adres_status,
		CASE WHEN COALESCE(p6.opt_out,'f') = 'f' THEN 'JA' WHEN p6.opt_out = 't' THEN 'NEEN' ELSE 'JA' END email_ontvangen,
		CASE WHEN COALESCE(p6.opt_out_letter,'f') = 'f' THEN 'JA' WHEN p6.opt_out_letter = 't' THEN 'NEEN' ELSE 'JA' END post_ontvangen,
		p6.iets_te_verbergen,
		COALESCE(p6.deceased,'f') overleden,
		p6.email,
		CASE
			WHEN p6.gender = 'M' THEN 'Dhr.'
			WHEN p6.gender = 'V' THEN 'Mevr.'
			ELSE pt.shortcut
		END aanspreking,
		p6.gender AS geslacht,
	 	pct.name,
		REPLACE(REPLACE(REPLACE(aml.name,';',','),chr(10),' '),chr(13), ' ') as description,
		aml.ref,
		--aaa.code,
	...
	FROM myvar v, account_move am
		...
		LEFT OUTER JOIN res_partner p6 ON p6.id = p.active_partner_id

		JOIN res_company rc ON aml.company_id = rc.id 
		LEFT OUTER JOIN res_country_state cs ON cs.id = p6.state_id
		LEFT OUTER JOIN res_country c ON p6.country_id = c.id
		LEFT OUTER JOIN res_country_city_street ccs ON p6.street_id = ccs.id
		LEFT OUTER JOIN res_country_city cc ON p6.zip_id = cc.id
		LEFT OUTER JOIN res_partner_title pt ON p6.title = pt.id
		--afdeling vs afdeling eigen keuze
		LEFT OUTER JOIN res_partner a ON p6.department_id = a.id
		LEFT OUTER JOIN res_partner a2 ON p6.department_choice_id = a2.id
		--link naar partner		
		LEFT OUTER JOIN res_partner a5 ON p6.relation_partner_id = a5.id
	 	--link naar rechtspersoon
	 	LEFT OUTER JOIN res_partner_corporation_type pct ON pct.id = p6.corporation_type_id
	WHERE p.active = 'false' AND COALESCE(p6.id,0) > 0
		AND (aa.code = '732100' OR  aa.code = '732000')
		AND aml.date BETWEEN v.startdatum AND v.einddatum
		--AND (p.active = 't' OR (p.active = 'f' AND COALESCE(p.deceased,'f') = 't'))	--van de inactieven enkele de overleden contacten meenemen
		--AND p.id = 387369
	ORDER BY p6.id, aml.date	
	);
--/*
--------------------------------------------------
