...
--WHERE LOWER(description) LIKE '%koalect by mangopay%' 
...
WHERE partner_id = 65656 ORDER BY date
--WHERE bron = 'npca'
...
--SELECT SUM(amount)
FROM
	(SELECT partner_id, geboorte_datum, NULL aard_vd_schenker,  aanspreking, geslacht, naam, achternaam, voornaam, naam_partner, straat, huisnummer, bus, postcode, gemeente woonplaats, land, SUM(credit) - SUM(debet) amount, NULL nr, adres_status, email_ontvangen, post_ontvangen, overleden, vzw
	FROM tempGIFTEN
	...
	GROUP BY partner_id, geboorte_datum, aanspreking, geslacht, naam, achternaam, voornaam, naam_partner, straat, huisnummer, bus, postcode, gemeente, land, adres_status, email_ontvangen, post_ontvangen, overleden, vzw) x
WHERE amount >= 40 
*/
...
------------------------
--/*
SELECT DISTINCT max(date) date, partner_id, aanspreking, geslacht, naam, voornaam, achternaam, straat, huisnummer, bus, postcode, gemeente, provincie, afdeling, overleden, adres_status, email_ontvangen, post_ontvangen 
FROM tempGIFTEN 
...
--WHERE NOT(project_code LIKE '%3333%')
GROUP BY partner_id, aanspreking, geslacht, naam, voornaam, achternaam, straat, huisnummer, bus, postcode, gemeente, provincie, afdeling, overleden, adres_status, email_ontvangen, post_ontvangen
ORDER BY postcode, achternaam, voornaam --WHERE partner_id IN ('94626','19544')
...
---------------------------------------------------------------
SELECT partner_id, aanspreking, geslacht, naam, voornaam, achternaam, straat, huisnummer, bus, postcode, gemeente, provincie, afdeling, overleden, adres_status, email_ontvangen, post_ontvangen
FROM myvar v, tempGIFTEN g	
WHERE project_code LIKE '%3333%'
	--AND g.date >= v.startdatumbosvooriedereen
	AND adres_status <> '2' AND post_ontvangen = 'JA' AND overleden = 'false'
GROUP BY partner_id, voornaam, achternaam, opzegdatum, straat, huisnummer, bus, postcode, gemeente, land, email, geslacht, aanspreking, project
---------------------------------------------------------------
