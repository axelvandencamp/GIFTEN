-- Function: public._crm_marketinginkomsten(date, date)

-- DROP FUNCTION marketing._crm_marketinginkomsten(text, date, date, integer);

CREATE OR REPLACE FUNCTION marketing._crm_marketinginkomsten(
	IN prm_periode text,
	IN prm_startdatum date,
	IN prm_einddatum date,
	--IN prm_partner_id integer,
	IN prm_sproc_naam integer DEFAULT 10,  
    --
    OUT account_id integer,
    OUT date date,
    OUT jaar double precision,
    OUT maand double precision,
...
BEGIN
	...
	WHERE aa.code IN ('740500','734000','732100','732000')
		AND aml.date BETWEEN v_startdatum AND v_einddatum
		--AND (CASE 
	...
	WHERE pph.project_nbr > '0' 
		AND pph.date BETWEEN v_startdatum AND v_einddatum
		/*AND (CASE 
			 	WHEN COALESCE(prm_partner_id,0) > 0 THEN p.id = prm_partner_id ELSE p.id > 0 END)*/
	--ORDER BY pph.date
;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION marketing._crm_marketinginkomsten(text, date, date, integer)
  OWNER TO axelvandencamp;
GRANT EXECUTE ON FUNCTION marketing._crm_marketinginkomsten(text, date, date, integer) TO public;
GRANT EXECUTE ON FUNCTION marketing._crm_marketinginkomsten(text, date, date, integer) TO axelvandencamp;
GRANT EXECUTE ON FUNCTION marketing._crm_marketinginkomsten(text, date, date, integer) TO readonly;


-- SELECT * FROM marketing._crm_marketinginkomsten(NULL,'2009-01-01','2009-12-31')
